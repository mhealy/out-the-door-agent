...
Criteria interpretation remains a fixture implementation limited to the canonical Hyundai Tucson Hybrid vocabulary. Quote analysis and follow-up wording require an OpenAI API key. Outbound messaging is limited to one buyer-selected dealer/vehicle workflow at a time, immutable drafts, fictitious contacts, and a side-effect-free fixture provider. Comparison covers only the existing dealer runs known to the current browser workspace; there is no persisted purchase-level grouping or automatic multi-dealer outreach. The demo has one deterministic response per interaction, so a sent follow-up truthfully stops at `WAITING_FOR_EXTERNAL_RESPONSE`; it does not fabricate a second response. There is no Gmail transport, draft editing, automatic background resume, durable outbox reconciliation, or event streaming.
